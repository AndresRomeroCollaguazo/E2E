...
@ShoppingCartEmpty
  Scenario: 2 - Cuando la cesta esté vacía de productos, no procesar la compra
    And da click en el icono del carrito
    Then el sistema deberia mostrar la cesta sin productos

@ShoppingCartProcess
  Scenario: 3 - Cuando se quiera procesar la compra, se mostrará un mensaje de compra exitosa
    And agregar productos al carrito de compras
    And da click en el icono del carrito
